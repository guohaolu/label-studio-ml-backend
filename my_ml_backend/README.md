...
- **手段（AC）**：从 Doris 拉取 `buyerName`，构建与快递面单相同的 **Aho-Corasick 自动机**，在正文中做**子串**扫描（`iter_buyer_nickname_matches` 内与 `pyahocorasick` 的 `iter` 一致）。
...
- **数据来源 / 契约**：表名 ``DORIS_BUYER_NICKNAME_TABLE``（未设置时默认为 `furniture_trade__trade_order_line_s_o`）、字段 `buyerName`；``DORIS_DATABASE`` 或表名中带库名。其它 Doris 相关变量与 `express_ac._fetch_dictionary` 一致。
